...
public final class ProjectPaths {
    ...
    public ProjectPaths(Parser parsedObjects) {
        ...
        for (String identifier : identifierToIdMap.keySet()) {
            identifierBIDPaths.addAll(identifierToIdMap.get(identifier).stream().map(id -> Paths.get(getRootPath().toString(), identifier, "BID_" + id.toString())).collect(Collectors.toList()));
        }
        ...
        for (int iteration : IntStream.rangeClosed(1, iterationNumber).toArray()) { // Will need to fix this again
            iterationsPaths.addAll(identifierBIDPaths.stream().map(path -> Paths.get(path.toString(), "Iterations", "_" + iteration)).collect(Collectors.toList()));
        }
        ...
        for (String mutationOperator : (ArrayList<String>) parsedObjects.getGp().getMutationOperators()) {
            mutationOperatorPaths.addAll(iterationsPaths.stream().map(path -> Paths.get(path.toString(), mutationOperator)).collect(Collectors.toList()));
        }
    ...
    public static final class PathVariables {
        ...
        public void setVariables(Path path) {
            ArrayList<String> variables = new ArrayList<>(Arrays.asList(path.toString().split(Pattern.quote(File.separator))));
